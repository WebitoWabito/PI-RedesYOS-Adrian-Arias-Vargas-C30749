...
void propagateLocalUpdate() {
    std::string csv = localFiguresCSV();
    std::string msg = "P/P/" + csv;
    for (const auto& bc : OTHER_BC) {
        ...
        dest.sin_family = AF_INET;
        dest.sin_port   = htons(DISCOVERY_PORT);
        inet_pton(AF_INET, bc.c_str(), &dest.sin_addr);
        sendto(fd, msg.c_str(), msg.size(), 0, (struct sockaddr*)&dest, sizeof(dest));
        close(fd);
    }
    log("[PUSH] P/P/ enviado a todas las islas (" +
        std::to_string(localFigures.size()) + " figuras)");
}
...
void udpListenerThread() {
    ...
    while (true) {
        ...
        if (msg.rfind("P/F/", 0) == 0) {
        ...
        } else if (msg.rfind("P/P/", 0) == 0) {
            std::cout << "[FORK ACTUALIZADO] " << srcIP << " notificó cambios "
                      << "en su listado de figuras." << std::endl;
            std::string data = msg.substr(4);
            auto names = split(data, ',');
            ...
            log("[UDP-LISTEN] P/P/ recibido de " + srcIP +
                ", mapa actualizado (" + std::to_string(routeMap[srcIP].size()) + " figuras)");
        }
    }
    close(fd);
}
...
void handleClient(VSocket* client) {
    ...
    //POST /figura/<nombre>  (sigue el protocolo P/K/<nombre>|<piezas>)
    if (method == "POST" && path.find("/figura/") == 0) {
        ...
        try {
            ...
            if (resp.find("ERROR") != std::string::npos) {
                ...
                std::thread(propagateLocalUpdate).detach();
    ...
    //DELETE /figura/<nombre>  (sigue el protocolo P/E/<nombre>)
    if (method == "DELETE" && path.find("/figura/") == 0) {
        std::string name = getFigureName(path);
        std::string proto = "P/E/" + name;

        log("[DELETE] Eliminando figura: " + name);
        ...
        try {
            ...
            if (resp.find("ERROR") != std::string::npos) {
                ...
                log("[DELETE] Figura eliminada: " + name);
                std::thread(propagateLocalUpdate).detach();
                client->Write(buildResponse(200, "Eliminada: " + name).c_str());
            }
        } catch (...) {
            client->Write(buildResponse(500, "Servidor de figuras no disponible").c_str());
        }
