...
size_t Socket::Read( void * buffer, size_t size ) {
   ...
   if (st == -1) {
      throw std::runtime_error( "Socket::Read( void *, size_t ) failed" );
   }
...
size_t Socket::Write( const void * buffer, size_t size ) {
   ...
   if (st == -1) {
      throw std::runtime_error( "Socket::Write( void *, size_t ) failed" );
   }
...
size_t Socket::Write( const char * text ) {
   ...
   if (st == -1) {
      throw std::runtime_error( "Socket::Write( char * ) failed" );
   }
