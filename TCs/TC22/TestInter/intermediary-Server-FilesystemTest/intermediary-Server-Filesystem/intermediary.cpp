...
// como necesitamos el socket antes y lo puse después, se declara desde acá
...
std::string getFigureFromFork(const std::string& ip, const std::string& figureName) {
    ...
    int fd = createUdpSocket(0, true);//socket sin bind fijo
    if (fd < 0) {
        std::cout << "[ERROR] No se pudo crear socket UDP para contactar " << ip << std::endl;
        return "";
    }

    //si el fork no responde se asume caído
    struct timeval tv{};
